...
namespace MediumMapperApi.Service.RabitMqGlobalData
{
    public class RabitMqGlobalDataClass
    {
        ...
        {
            await ToAddchanel.QueueDeclareAsync(queue: QueueName, durable: false, exclusive: false, autoDelete: false, arguments: null);
        }
        ...
        public async Task CreateConsumerForChanelToQueue(IChannel ToAddchanel, string QueueName) 
        {
            ...
            consumer.ReceivedAsync += async (ch, ea) => 
            {
                ...
                await _Mediator.Handler(body, (BasicProperties)ea.BasicProperties);
        ...
        public async Task SendMessageWrapper(TempMessageDTO Message,string sendQueue, BasicProperties? props)
        {
            
            byte[] body = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(Message));
            IChannel Sendchanel = _sendChannel == null? await GetSendChannelToRabit() : _sendChannel;
            await CreateNewQueueForChanel(Sendchanel, sendQueue);
            await Sendchanel.BasicPublishAsync(exchange: "",
